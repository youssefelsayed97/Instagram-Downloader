...
class InstaloaderContext:
    ...
    def login(self, user, passwd):
        ...
        session = requests.Session()
        # print(session.cookies.get_dict()['mid'])
        session.cookies.update({'mid': '', 'ig_pr': '1',
                                'ig_vw': '1920', 'ig_cb': '1', 'csrftoken': '',
                                's_network': '', 'ds_user_id': ''})#'sessionid': '',
        session.headers.update(self._default_http_header())
        # Override default timeout behavior.
        # Need to silence mypy bug for this. See: https://github.com/python/mypy/issues/2427
        session.request = partial(session.request, timeout=self.request_timeout) # type: ignore
...
class RateController:
    ...
    exception when the rate limit is hit::
       ...
       class MyRateController(packages.RateController):
           def sleep(self, secs):
               raise MyCustomException()

       L = packages.Instaloader(rate_controller=lambda ctx: MyRateController(ctx))
    """
