...
class Profile:
    ...
    @property
    def threads_username(self) -> str:#Youssef Elsayed
        if not self._metadata('text_post_app_badge_label'):
            return ""
        threads_username = f"@{self._metadata('text_post_app_badge_label')}"
        return threads_username

    @property
    def mutual_followers_ids(self) -> list[int]:#Youssef Elsayed:
        if not self._metadata("profile_context_facepile_users"):
            return []
        ids = []
        for id in self._metadata("profile_context_facepile_users"):
            ids.append(id['id'])
        return ids

    @property
    def mutual_followers_count(self) -> int:#Youssef Elsayed
        if not self._metadata("mutual_followers_count"):
            return 0
        return self._metadata("mutual_followers_count")

    @property
    def mutual_followers_usernames(self) -> list[str]:#Youssef Elsayed
        mutual = self._metadata("profile_context_links_with_user_ids") or []
        # print("METADATA:", self._metadata())
        # print("NODE:", self._node)
        # print(mutual)
        return [user.get("username") for user in mutual or [] if user.get("username")]

    @property
    def bio_links(self) -> list[str]:#Youssef Elsayed
        # if not self._metadata('bio_links'):
        #     return []
        #links = []
        # for link in self._metadata('bio_links'):
        #     if link['url'] != "":
        #         links.append(link['url'])
        # return links
        links = self._metadata("bio_links") or []
        return [link['url'] for link in links if link.get('url')]
    @property
    def linked_fb_info_name(self) -> str:#Youssef ELsayed
        if not self._metadata('linked_fb_info'):
            return ""
        return self._metadata('linked_fb_info', 'linked_fb_user', 'name')

    @property
    def linked_fb_info_url(self) -> str:#Youssef Elsayed
        if not self._metadata('linked_fb_info'):
            return ""
        return self._metadata('linked_fb_info', 'linked_fb_user', 'profile_url')
...
def get_json_structure(structure: JsonExportable) -> dict:
    ...
    return {
        ...
        'packages': {'version': __version__, 'node_type': structure.__class__.__name__}
    }
...
def load_structure(context: InstaloaderContext, json_structure: dict) -> JsonExportable:
    ...
    .. versionadded:: 4.8
    """
    if 'node' in json_structure and 'packages' in json_structure and \
            'node_type' in json_structure['packages']:
        node_type = json_structure['packages']['node_type']
        if node_type == "Post":
